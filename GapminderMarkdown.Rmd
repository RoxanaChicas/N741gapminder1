...
[https://github.com/RoxanaChicas/N741gapminder1](https://github.com/RoxanaChicas/N741gapminder1) which was typed in to the Rmarkdown file as `[https://github.com/RoxanaChicas/N741gapminder1
](https://github.com/RoxanaChicas/N741gapminder1)`.
...
The standard deviation life expectancy is `r round(sd(gapminder$lifeExp), digits=2)` years.

The median life expectancy is `r round(median(gapminder$lifeExp), digits=2)` years.

The sample size life expectancy is `r round(length(gapminder$lifeExp), digits=2)` years.
...
# bubble plot, focus just on Africa and Europe in 2007
keepers <- with(gap_with_colors,
                continent %in% c("America", "Asia") & year == 1992)
plot(lifeExp ~ gdpPercap, gap_with_colors,
     subset = keepers, log = "x", pch = 21,
     cex = sqrt(gap_with_colors$pop[keepers]/pi)/1500,
     bg = gap_with_colors$cc[keepers])
